...
const unsigned long breathInterval = 30; // ms

// Game colors (strong, vibrant colors)
const uint32_t gameColors[] = {
  strip.Color(255, 0, 144),    // Vivid Magenta (Bright Pinkish Purple)
  strip.Color(0, 255, 255),    // Bright Cyan (Electric Blue)
  strip.Color(255, 255, 0),    // Bright Yellow
  strip.Color(0, 255, 0),      // Bright Lime Green
  strip.Color(255, 85, 0)      // Bright Orange-Red
};


const uint32_t fadeColor = strip.Color(0, 0, 0);  // Off between inputs
const uint32_t scoreColor = strip.Color(0, 50, 255);  // Blueish for score display

// Game variables
int sequence[31];  // Store up to 31 because that is the max binary  11111     16 8 4 2 1
